...
class FirstAidEntity extends Entity {
	private final healAmount:Int = GlobalGameData.fakHealAmount;
	...
	function createSprite(x:Float, y:Float) {
		...
		sprite.x = x;
		sprite.y = y;
		sprite.timeline.gotoAndStop(104);
	...
	function overlapPlayer(col1:ICollider, col2:ICollider) {
		GlobalGameData.playerEntity.heal(healAmount);
		destroy();
	}

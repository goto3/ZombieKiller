...
import customCollisionEngine.Sides;
...
class CollisionBox extends Body implements ICollider {
	...
	public function overlap(collider:ICollider, ?NotifyCallback:ICollider->ICollider->Void):Bool {
		if (collider.collisionType() == CollisionType.Box) {
			...
			if (inline overlapVsCircle(cast collider)) {
				if (NotifyCallback != null)
					NotifyCallback(this, collider);
				return true;
			}
	...
	public function areTouching(box:CollisionBox):Bool {
		return (Math.floor(box.x) <= Math.floor(x + width)
			&& Math.floor(box.x + box.width) >= Math.floor(x)
			&& Math.floor(box.y) <= Math.floor(y + height)
			&& Math.floor(box.y + box.height) >= Math.floor(y));
	}

	function overlapVsCircle(circle:CollisionCircle):Bool {
		var boxVerticeX:Float = 0;
		var boxVerticeY:Float = 0;
		if (circle.x < this.x)
			boxVerticeX = this.x;
		else
			boxVerticeX = this.x + this.width;
		if (circle.y < this.y)
			boxVerticeY = this.y;
		else
			boxVerticeY = this.y + this.height;

		var distX:Float = circle.x + circle.radius - boxVerticeX;
		var distY:Float = circle.y + circle.radius - boxVerticeY;
